...
class account_bank_statement_wizard(models.TransientModel):
    ...
    bank_opt = fields.Selection([('santander','Santander'),('estado','Banco Estado'),('chile','Banco de Chile'),('itau','Banco Itau'),('bci','Bci')])
                    ...
                    if row_no>4:
                        try:
                            if line[4]!='':
                                ...
                                fecha=line[0]
                                fecha=float(fecha)
                                fecha=xlrd.xldate_as_tuple(fecha,workbook.datemode) 
                                year=fecha[0]
                                month=fecha[1] if len(str(fecha[1]))==2 else '0'+str(fecha[1])   
                                day=fecha[2] if len(str(fecha[2]))==2 else '0'+str(fecha[2])
                                fecha_str = str(day)+'/'+str(month)+'/'+str(year)
                                date_string = datetime.strptime(fecha_str, '%d/%m/%Y').strftime('%Y-%m-%d')
                                values.update( {'date':date_string,
                                                'ref': line[3].replace('.0',''),
                                                'partner': '',
                                                'memo': line[2].decode("utf-8"),
                                                'amount': int(line[4].replace('.0',''))*-1,
                                            })
                            else:
                                fecha=line[0]
                                fecha=float(fecha)
